...
class MLP(MLModel):
    ...
    def fit(self, X_train, y_train, plot=True, weights=None):
        print(f"Training MLP with params: {self.params}")
        history = self.model.fit(X_train, y_train, epochs=self.params['epochs'], batch_size=self.params['batch_size'], validation_split=self.params['validation_split'], sample_weight=weights)
        if plot:
